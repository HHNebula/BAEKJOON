...
//		첫째 줄에 M개의 문자열 중에 총 몇 개가 집합 S에 포함되어 있는지 출력한다.
		...
		String[] retention = new String[retentionSize];
		String[] inspection = new String[inspectionSize];

		for (int i = 0; i < retentionSize; i++) {
			retention[i] = br.readLine();
		}

		for (int i = 0; i < inspectionSize; i++) {
			inspection[i] = br.readLine();
		}
		
		for (String str1 : inspection) {
			for(String str2 : retention) {
				if (str1.equals(str2)) {
					result++;
				}
			}
